...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    # 加载优化后的 pose 文件
    pose_optimized_path = "/home/logan-kyo/InstantSplat/output_eval_XL/sora/Santorini/3_views/pose/ours_1000/pose_interpolated.npy"
    optimized_poses = np.load(pose_optimized_path)  
    
     # 指定 refined GT 图像所在的文件夹路径
    refined_gt_folder = "/home/logan-kyo/InstantSplat/output_eval_XL/sora/Santorini/3_views/interp/ours_1000/refined"
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        pose = torch.tensor(optimized_poses[viewpoint_cam.colmap_id - 1], device='cuda')
        ...
        # gt_image = viewpoint_cam.original_image.cuda()
         # 修改 2：从 refined 文件夹中加载监督的 GT 图像
        
        refined_gt_path = os.path.join(refined_gt_folder, viewpoint_cam.image_name)
        if not os.path.exists(refined_gt_path):
            raise FileNotFoundError(f"Refined GT image not found: {refined_gt_path}")
        gt_np = imageio.imread(refined_gt_path)
        # 将 (H, W, 3) 的 numpy 数组转换为 (3, H, W) 的 tensor，并归一化到 [0, 1]
        refined_gt = torch.from_numpy(gt_np).permute(2, 0, 1).float().div(255.0).to("cuda")
        gt_image = refined_gt
