...
class EulerDiscreteScheduler(SchedulerMixin, ConfigMixin):
    ...
    def step(
        ...
        if guided and video_latents is not None:
            pred_original_sample = self.fuse_latents(pred_original_sample, video_latents, mask)
    ...
    def fuse_latents(self, pred_original_sample, video_latents, mask):
        '''
        pred_original_sample: [B, frames, D=4, H, W]
        video_latents: [frames, D=4, H, W]
        '''
        if mask is None:
            return video_latents.unsqueeze(0)

        mask = resize(mask, pred_original_sample.shape[-2:]).unsqueeze(1).unsqueeze(0)
        video_latents = video_latents.unsqueeze(0)

        pred_original_sample = video_latents*mask + pred_original_sample*(1-mask)
        # pred_original_sample = (pred_original_sample + video_latents[None,...])/2
